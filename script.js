...
function renderSimulator() {
  ...
  const projectedValue = futureValueWithDailyContributions(
    dailyContribution,
    destination.apy,
  );
  ...
  setText(
    summaryMonthly,
    `${formatCurrency(monthlyContribution)}/mo projected`,
  );
  ...
  try {
    ...
    if (data.ok) {
      if (data.duplicate) {
        ...
        setFeedback(
          "You're in. Helio beta access will land in your inbox.",
          'success',
        );
      }
      waitlistForm.reset();
